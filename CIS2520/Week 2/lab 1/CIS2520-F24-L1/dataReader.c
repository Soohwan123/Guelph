		...
		)
{
	...
	*delimiterPosition = '\0';
	 ...
	 */
	cleanedValue = stripNonPrinting(&delimiterPosition[1]);
	...
	//value = cleanedValue; ---> chr* is read only. So you need to use the function strcpy.
	strcpy(value, cleanedValue);


	printf("DBG: \"read\" - key/value are '%d/ %s'\n",
			*key, value);
